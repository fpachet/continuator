...
The singleton policy is parameterized:

- `acceptance_probability=None` keeps the classic probability `1 / (order + 1)`.
- A numeric `acceptance_probability` in `[0, 1]` forces a fixed accept rate.
- `min_singleton_order` controls the lowest order where singleton skipping is
  allowed.
- `suppress_skipped_symbol` controls whether the skipped symbol is masked at
  intermediate lower orders when alternatives exist.
...
chosen transition. Trace steps also expose the policy name, candidate orders,
...
position. Feasible lengths inside the requested window are weighted by BP path
mass. Once a length is selected, the sequence itself is generated by the same
stepwise order policy used by fixed-length constrained sampling.

Use `continue_until_end_with_trace(...)` to return both the generated sequence
and its per-step `SampleStep` trace.

## MIDI Comparison Script

The repository includes a small comparison script for listening tests:

```bash
python examples/compare_classic_context_bp_midi.py data/prelude_c.mid --samples 3
```

It learns the same MIDI file or folder into `ClassicContinuator` and
`ContextBPContinuator`, prints generated pitch summaries, and can optionally
write MIDI files with `--output-dir`.
